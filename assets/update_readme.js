...
function parseTrackingFile(contents) {
    ...
    for (const rawLine of contents.split(/\r?\n/)) {
        ...
        if (!match) {
            console.warn(
                `Ignoring malformed name_tracking.txt line: ${rawLine}`
            );
            continue;
        }
...
function parseCards(xml, tracking) {
    ...
    for (const cardXml of cardMatches) {
        ...
        const sets =
            cardXml.match(/<set\b[^>]*>[\s\S]*?<\/set>/gi) || [];
        ...
        for (const setXml of sets) {
            ...
            if (!flavorName || !picurl || !uuid) continue;

            const seriesName = SERIES_NAMES[flavorName];

            if (!seriesName) {
                console.warn(
                    `Skipping "${name}": unknown flavorName "${flavorName}".`
                );
                continue;
            }
            ...
            if (!suffix) {
                console.warn(
                    `Skipping "${name}": invalid UUID "${uuid}".`
                );
                continue;
            }
            ...
            if (!trackedName) {
                console.warn(
                    `Skipping "${name}" UUID ${suffix}: no entry in name_tracking.txt.`
                );
                continue;
            }
...
function findSeriesHeading(lines, category, series) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (inCategory && lines[i].startsWith('## ')) {
            return -1;
        }
...
function insertIntoSeries(
    ...
    for (let i = headingIndex + 1; i < end; i++) {
        ...
        const existingRelease =
            bulletReleaseMap.get(lines[i]);

        if (
            existingRelease !== undefined &&
            existingRelease > card.release
        ) {
...
function incrementSeriesCount(
    ...
    const headingIndex =
        findSeriesHeading(lines, category, series);
...
function main() {
    ...
    const rawCards =
        parseCards(xml, tracking);

    const cards = groupDfcCards(rawCards);

    cards.sort(
        (a, b) => a.release - b.release
    );

    const bulletReleaseMap =
        buildBulletReleaseMap(cards);

    let lines =
        originalReadme.split(/\r?\n/);
    ...
    console.log(
        `Added ${added.length} new entr${added.length === 1 ? 'y' : 'ies'}:`
    );
    ...
    for (const card of added) {
        if (card.isDfc) {
            ...
            console.log(
                `  ${card.name} [${card.uuidSuffix}] = ${card.trackedName} (${card.seriesName})`
            );
        }
    }
